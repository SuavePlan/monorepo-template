...
- **Known, accepted limitations** (found in round 5 and the final whole-implementation review, deliberately not fixed — not chasing every markdown-hiding mechanism, per the review-round cap): `NONE_IDENTIFIED_RE`'s `[-—]` class doesn't include an en dash (`–`, U+2013) — a genuine en-dash (a common editor auto-substitution) would fail to register `noneIdentified: true` even with a real reason present, surfacing as a normal "section present but invalid" violation an author can fix by retyping the dash. `stripCodeBlocks` doesn't specially handle a malformed/unclosed fence or HTML comment (a stray ` ``` ` or `<!--` with no matching close before EOF) — behavior in that case is unspecified/implementation-dependent rather than deliberately designed, since malformed markdown in a `proposal.md` is itself already a defect an author would need to fix regardless of this gate. And the 30-line frontmatter bound (shared with the pre-existing `**Package**:` convention in `conformance.ts`) means a proposal whose `**App**:`/`**Package**:` line falls past line 30 of the (stripped) document is silently skipped entirely — not flagged as a violation, just invisible to this gate — so an unusually long title/preamble block could bypass §22 entirely rather than trigger it; authors are expected to keep frontmatter fields within the first 30 lines, matching every other gate in this repo that relies on the same convention.
