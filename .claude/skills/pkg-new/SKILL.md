...
3. Confirm there is an active OpenSpec change for this package (`openspec/changes/` contains a relevant entry per `CLAUDE.md` §2). If not, ask: "No OpenSpec change found for this package. Should I create one first?"
...
```json
{
  ...
  "devDependencies": {
    ...
    "@suaveplan/typescript-config": "^0.1.0",
    "@suaveplan/vite-config": "^0.1.1",
...
- One `exports` subpath **per feature module** (`CLAUDE.md` §6.3) — add one for every folder under `src/`, not just a single `"."` barrel. `dist/<feature>/<feature>.js` exists per-module because `@suaveplan/vite-config`'s `defineLibConfig` builds with `preserveModules: true`.
...
- Adjust deps to what the package actually needs; remove unused ones. If it depends on another package that lives in *this repo's own* `packages/` tree, that one genuinely is `workspace:^` — only the `@suaveplan/*` foundation packages above are external.

### `tsconfig.lib.json`

```json
{
  "extends": "@suaveplan/typescript-config/library.json",
  "compilerOptions": {
    "rootDir": "src",
    "outDir": "dist"
  },
...
```json
{
  "extends": ["@suaveplan/biome-config/biome.json"],
  "root": false
}
```
...
- [ ] `@suaveplan/*` foundation deps are pinned versions, **not** `workspace:^` (this repo is a template, not the origin monorepo)
